...
\operacion{Vacio}{}{res : diccNat(\kappa, \sigma)}
 ...
 \operacion{Claves}{in d:diccNat(\kappa \sigma)}{res:conjLog(\kappa)}
 {$true$}
 {$res \igobs claves(d)$}
...
diccNat
\serc{raiz: puntero(estr(\kappa\, \sigma))}{
	\\
	\donde{estr(\kappa, \sigma)}{\tupla{ clave: \kappa, significado:\sigma, hijoDer: puntero(estr(\kappa \sigma)), hijoIzq: puntero(estr(\kappa \sigma)), diccClaves: conjLog(\kappa)}}}
...
\algoritmo{iDefinir}{in/out d:estr, in n:\kappa, in s:\sigma}{}{
  ...
  \State $AgregarRapido(d.diccClaves, n)$ \complejidad{O(m)}
}{O(m)}

\algoritmo{iBorrar}{in/out d:estr, in n:\kappa}{}{
  \If {$d == NULL$}
    \State $Fin Funcion$
    \complejidad{O(1)}
  ...
  \EndIf
  \State $Eliminar(d.diccClaves, n)$ \complejidad{O(m)}
