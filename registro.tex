...
\serc{DiccTrie}{
  \donde{DiccTrie}{\tupla{
      ...
      significados: \mbox{DiccTrie(campo, dato)},
    }
  }
}
...
\rep{dicc}{d}{true $\Leftrightarrow$ $\#$claves(secuADicc($d$.claves)) $=$ long($d$.claves) $\land$ long($d$.claves) = long($d$.significados)}
...
\algoritmo{agregarCampos}{in $r_1$: reg in $r_2$: reg}{res:}{
  ...
  \algoritmo{copiarCampos}{in cc: conj(campo) in/out r1: reg, in r2: reg}{}{
    ...
    \While {haySiguiente(itcc)}; \complejidad{O(1)}
                    \State definirRapido(siguiente(itcc), obtener(siguiente(itcc)\,r2), r1); \complejidad{O(1)}
