...
\algoritmo{coincidencias}{in crit:registro, in cr:Conj(registro)}{res:Conj(ItConj(registro))}{
 	\State Conj(registro) salida	$\larr$ Vacio() \complejidad{$\theta$(1)}
	...
	\State y agregarlos al conjunto de registros salida
  	...
  	\While{HaySiguiente?(cr)}							\complejidad{$\theta$(Cardinal(cr))}
		\If {coincidenTodos(crit,campos(crit),Siguiente(itcr))}  \complejidad{$\theta$(1)}
				\State	AgregarRapido(salida, CrearIt(Siguiente(itcr))) \complejidad{$\theta$(1)}
				
				\State	AgregarRapido(salida, itcr) \complejidad{$\theta$(1)}

		\EndIf				
		\State	Avanzar(itcr) \complejidad{$\theta$(1)}
	\EndWhile   
	\State res $\larr$ salida\complejidad{$\theta$(1)por ref}
}{O(Cardinal(cr))}
...
\algoritmo{combinarRegistros}{in c:campo, in cr1:Conj(registro), in cr2:Conj(registro)}{res:Conj(registros)}{
	...
	\If{Cardinal(cr2)$\geq$1}\complejidad{$\theta$(1)}  
		Registro rtemp $\larr$ Siguiente(CrearIt(cr2))\complejidad{$\theta$(1)}  
		Tipo rac $\larr$ Tipo?(Significado(rtemp, c))\complejidad{$\theta$(1)}  
		\If{rac} \complejidad{$\theta$(1)}  
			\State	Caso Natural
			\State	DiccNat(Nat, Conj(registro)) d $\larr$ vacio() \complejidad{$\theta$(1)}  
			\State	itcr2 $\larr$ CrearIt(cr2)\complejidad{$\theta$(1)}  
			\While{HaySiguiente?(itcr2)}\complejidad{$\theta$(1)}  
		...
		\Else
			\State	Caso String
			\State	DiccString(String, Conj(registro)) d $\larr$ vacio() \complejidad{$\theta$(1)}  
			\State	itcr2 $\larr$ CrearIt(cr2)\complejidad{$\theta$(1)}  
			\While{HaySiguiente?(itcr2)}\complejidad{$\theta$(1)}  
					\State Dato valor $\larr$ Obtener(Siguiente(itcr2), c) \complejidad{O(1)}
		...
		\State res $\larr$ vacio()
	  	\While{HaySiguiente(itcr1)}         \complejidad{$\theta$(Cardinal(cr1))}
			\State	AgregarRapido(res, combinarTodos(c,Siguiente(itcr1),cr2)) \complejidad{$\theta$(vvv)}	
