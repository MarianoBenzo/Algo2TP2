...
\algoritmo{dameColumna}{in c:campo, in cr:Conj(registro)}{res:Conj(dato)}{
  	\State 	Conj(Dato)	cj $\larr$ vacio();				\complejidad{O(1) por ref}
...
%	\If{Cardinal(cr)$\geq$1}\complejidad{O(1) por ref}		
%			\State	Tvalor $\larr$ Tipo?(Significado(Siguiente(CrearIt(cr))), c)\complejidad{O(1) por ref}		
%			\If{Tvalor}
%					\State	DiccNat(nat,bool) cj $\larr$ Vacio()\complejidad{O(1) por ref}		
%			\Else
%					\State	ConjString(string) cj $\larr$ Vacio()\complejidad{O(1) por ref}		
%			\EndIf
%			\State 	itcr $\larr$ CrearItConj(cr)					\complejidad{O(1) por ref}		
...
%										\State 	AgregarRapido(cjd, data)\complejidad{O(1) por ref}
%										\State	AgregarRapido(cj, valorNat(data))\complejidad{O(1) por ref}
%								\EndIf	
%						\Else
%		  						\If {$\neg$Pertenece?(cj, valorString(data))}\complejidad{O(1) por ref}
%										\State 	AgregarRapido(cjd, data)\complejidad{O(1) por ref}
%										\State	AgregarRapido(cj, valorString(data))\complejidad{O(1) por ref}
%								\EndIf	
%						\EndIf
%				\State	Avanzar(itcr);							\complejidad{O(1) por ref}
%			\EndWhile     	
