...
\algoritmo{campos}{in t:tab}{res:ConjString(campo)}{
  \State $res \larr DiccClaves(t.Campos)$ \complejidad{O(1)}
...
\algoritmo{nuevaTabla}{in nombre: string, in claves: conj(campo), in columnas: registro}{res: tab}{
	...
	\State IndiceS $\larr <  c, False, Vacio(), d, d  >$ 		\complejidad{O(1)}
	\State IndiceN $\larr <  c, False, Vacio(), d, d  >$ 		\complejidad{O(1)}
...
\algoritmo{agregarRegistro}{in r: registro, in/out t: tab}{}{
	...
	\If{t.IndiceS.EnUso $\land$ t.IndiceN.EnUso}
		...
		\State Bool DefinidoS $\larr$Definido?(t.IndiceS.Indice, ValorString(valorS))
		\State Bool DefinidoN $\larr$Definido?(t.IndiceN.Indice, ValorNat(valorN))
		\If{$\neg$DefinidoS $\land$ $\neg$DefinidoN}
			\State Ambos no estan definidos
			\State cjS $\larr$ vacio()
			\State cjN $\larr$ vacio()
			\State newS $\larr$AgregarRapido(cjS, nuevo)			
			\State newN $\larr$AgregarRapido(cjN, nuevo)			
			\State Definir(t.IndiceS.Indice, ValorString(valorS), cjS)
			\State Definir(t.IndiceN.Indice, ValorString(valorN), cjN)
		\Else
			\If{DefinidoS $\land$ $\neg$DefinidoN}
				\State cjS $\larr$ Significado(t.IndiceS.Indice, ValorString(valorS))
				\State cjN $\larr$ vacio()
				\State newS $\larr$AgregarRapido(cjS, nuevo)			
				\State newN $\larr$AgregarRapido(cjN, nuevo)			
				\State Definir(t.IndiceN.Indice, ValorNat(valorN), cjN)
			\Else
				\If{$\neg$DefinidoS $\land$ DefinidoN}
					\State cjN $\larr$ Significado(t.IndiceN.Indice, ValorNat(valorN))
					\State cjS $\larr$ vacio()
					\State newS $\larr$AgregarRapido(cjS, nuevo)			
					\State newN $\larr$AgregarRapido(cjN, nuevo)			
					\State Definir(t.IndiceS.Indice, ValorString(valorS), cjS)
		...
		\If{t.IndiceS.Min $>$ valorS} \complejidad{O(L)}
			\State t.IndiceS.Min $\larr$ valorS\complejidad{O(1)}
		\EndIf
		\If{valorS $>$ t.IndiceS.Max}\complejidad{O(L)}
			\State t.IndiceS.Max $\larr$ valorS\complejidad{O(1)}
		\EndIf	
		\If{t.IndiceN.Min $>$ valorN} \complejidad{O(1)}
			\State t.IndiceN.Min $\larr$ valorN\complejidad{O(1)}
		\EndIf
		\If{valorN $>$ t.IndiceN.Max}\complejidad{O(1)}
			\State t.IndiceN.Max $\larr$ valorN\complejidad{O(1)}
		\EndIf	
	\Else
		\If{t.IndiceS.EnUso $\land$ $\neg$t.IndiceN.EnUso}
			...
			\Else
				...
				\State itConj(registro) newS $\larr$AgregarRapido(viejo, nuevo)\complejidad{O(1)}
				\State itConj(registro) newN $\larr$CrearIt(vacio())
		...
		\Else
			\If{$\neg$t.IndiceS.EnUso $\land$ t.IndiceN.EnUso}
				...
				\Else
					\State Conj(Registro) viejo $\larr$ Vacio() \complejidad{O(1)}
					\State itConj(registro) newN $\larr$AgregarRapido(viejo, nuevo)\complejidad{O(1)}
					\State itConj(registro) newS $\larr$CrearIt(vacio())
					\State Definir(t.IndiceN.Indice, ValorNat(valor), viejo) \complejidad{O(L)}
					\State Como ingresamos un nuevo valor, actualizamos el min y max
					\If{t.IndiceN.Min $>$ valor} \complejidad{O(1)}
						\State t.IndiceN.Min $\larr$ valor\complejidad{O(1)}
					\EndIf
					\If{valor $>$ t.IndiceN.Max}\complejidad{O(1)}
...
\algoritmo{borrarRegistro}{in crit: registro,in/out t: tab}{}{
	...
	\State Caso: Hay algun indice en uso.
		...
		\Else
			...
			\State itConj(registro) cr $\larr$ CrearIt(t.registros)\complejidad{O(1)}
			...
			\While{HaySiguiente(cr) $\land$ $\neg$(valorm=valor)}\complejidad{O(Cardinal(t.registros))}
					...
					\State valorm $\larr$ Significado(Delr, c)		\complejidad{O(1)}
					\If{valorm=valor}
...
%	\State	La idea es no agregar el mismo dato dos veces, para eso uso un Dicc
...
%			\If{Tvalor}
%					\State	DiccNat(nat,bool) cj $\larr$ Vacio()\complejidad{O(1)}		
%			\Else
...
\algoritmo{mismosTipos}{in r:registro, in t:tab}{res:bool}{
  	...
  	\State 	itconjClaves $\larr$ CrearIt(Campo(r)) \complejidad{O(1)}
  	\While{valor $\yluego$ HaySiguiente?(itconjClaves)}				\complejidad{O(1)}
...
\algoritmo{BuscarEnTabla}{in criterio: registro, in t: tab}{res: Conj(ItConj(registro))}{
	...
	\State	en ese caso la complejidad es O(\#(Campos(criterio))+L + Log(n))
