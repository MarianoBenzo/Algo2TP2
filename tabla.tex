...
\sexc{tabla}
$\textbf{usa}$  
\generos{nat, dato, campo, tipo, registro, conjString, string, diccString(string, alfha), diccAVL(nat, beta)}
...
\operacion{claves}{in t: tab }{res : itConjString(campo)}
 {true}
 {$res \igobs claves(t)$}
...
\operacion{indices}{in t: tab }{res : itConjString(campo)}
 {true}
 {$res \igobs indices(t)$}
...
\operacion{campos}{in t: tab }{res : itConjString(campo)}
 {true}
 {$res \igobs campos(t)$}
...
%%%%%% Otras Operaciones %%%%%%%%%%%
 ...
 {Retorna res por referencia.} 
  ...
  \operacion{hayCoincidencia}{in r: registro,in cj1: ConjString(campo), in cj2: Conj(registro)}{res:bool}
 {$True$}
...
\algoritmo{nuevaTabla}{in nombre: string, in claves: conj(campo), in columnas: registro}{res: tab}{
	...
	\State Dato d $\larr$ Obtener(columnas, DameUno(claves))
	\State IndiceS $\larr <  DameUno(claves), False, Vacio(), d, d  >$ 		\complejidad{O(1)}
	\State IndiceN $\larr <  DameUno(claves), False, Vacio(), d, d  >$ 		\complejidad{O(1)}
...
\algoritmo{borrarRegistro}{in crit: registro,in t: tab}{}{
	\State	Campo c $\larr$ DameUno(Campos(crit))	\complejidad{$\theta$(1)} %chequear
	\State 	Dato valor $\larr$	Significado(crit, c)			\complejidad{$\theta$(1)}
	...
	\If{t.IndiceS.EnUso $\land$ t.IndiceS.CampoI=c}
		...
		\If{Definido?(t.IndiceS.Indice, S)}
				\State	itConj(registro) itr $\larr$ Significado(t.IndiceS.Indice, S) 																																\complejidad{$\theta$(1)}
...
\algoritmo{dameColumna}{in c:campo, in cr:Conj(registro)}{res:Conj(dato)}{
	...
	\If{Cardinal(cr)$\geq$1}\complejidad{$\theta$(1)}		
			\State	Tvalor $\larr$ Tipo?(Significado(DameUno(cr), c))\complejidad{$\theta$(1)}		
			\If{Tvalor}
...
%    \State $posHippie \larr campus.hippies.Significado(nombre)$ \complejidad{O(long(nombre))}
