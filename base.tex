...
\algoritmo{agregarTabla}{in t: tabla, in/out b: estr}{}{
	\State	info\_tabla $\larr$ $\langle t.cantidadDeAccesos, t, vacio()\rangle$  \complejidad{O(1) por ref}
...
\algoritmo{generarVistaJoin}{in t1: string, in t2: string, in c: campo, in/out b: estr}{}{
	\State	Join $\larr$ vacio() \complejidad{O(1)}
	...
	\State	Conj(Dato) cjs $\larr$ vacio()
	\State	Conj(Dato) cjn $\larr$ vacio()
	\State	Cargo los valores de t1 en un dicc que depende del tipo del campo c
	\If{$\neg$tipoCampo(c, Ta1)} \complejidad{O(1)}
		...
		\While{HaySiguiente?(itcjd2)} \complejidad{O(m)}
			...
			\If{Def}
				AgregarRapido(cjs, Siguiente(itcjd2))\complejidad{O(1)}
		     \EndIf
			\State	Avanzar(itcjd1)\complejidad{O(1)}
		\EndWhile	
	\Else
		...
		\While{HaySiguiente?(itcjd2)} \complejidad{O(m*Log(n))}
			...
			\If{Def}
				AgregarRapido(cjn,Siguiente(itcjd2))\complejidad{O(1)}
		     \EndIf
