...
\algoritmo{hayJoin?}{in t1: string, in t2: string, in b: estr}{res: bool}{
	\State   res $\larr$ Pertenece?(b.Joins, $\langle t1, t2\rangle$  )$\lor$Pertenece?(b.Joins, $\langle t2, t1\rangle$ )\complejidad{O(1)}
}{O(1)}

\algoritmo{campoJoin}{in t1: string, in t2: string, in b: estr}{res: campo}{
   \If{Pertenece?(b.Joins, $\langle t1, t2\rangle$ )}\complejidad{O(1)}
				\State	res $\larr$ Obtener(e.Joins, $\langle t1, t2\rangle$).campoJ\complejidad{O(1)}
	\Else
				\State	res $\larr$ Obtener(e.Joins, $\langle t2, t1\rangle$).campoJ\complejidad{O(1)}
	\EndIf
}{O(1)}
%%%%%%%%%%%%% generadores %%%%%%%%%%%%%
\algoritmo{nuevaDB}{}{res: estr)}{ 
  \State res $\larr  \langle vacio(), vacio()\rangle$ \complejidad{O(1)}
}{O(1)}

\algoritmo{agregarTabla}{in t: tabla, in b: estr}{}{
	\State	info\_tabla $\larr$ $\langle t.cantidadDeAccesos, t\rangle$  \complejidad{O(1)}
...
\algoritmo{generarVistaJoin}{in t1: string, in t2: string, in c: campo, in/out b: estr}{}{
	...
	% Vemos si existe un join en ambas tablas para c
	\If{Definido?(T\_actual1.Indices, c) $\land$ Definido?(T\_actual2.Indices, c)}
			\State	ind1 $\larr$ Obtener(T\_actual1.Indices, c)
